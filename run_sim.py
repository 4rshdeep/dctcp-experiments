...
def experiment():
    ...
    throughputs = []
    K = 20 # K set to 20 in paper
        # Ks = [i for i in range(1,11,1)]
        # Ks += [i for i in range(15,35,5)]
        # Ks += [i for i in range(40,101,10)]
        # for K in Ks:
    out_q_file = congestion_alg + '_EXP_q_size.out'
            # run NS-2 simulation
    num_flows = 3
    link_cap = '100Mbps'
    link_delay = '10ms'
...
def main():
    ...
    parser.add_argument('--conv', action='store_true', help="reproduce figure 14 from the DCTCP paper")
    parser.add_argument('--parse', action='store_true', help="reproduce figure 14 from the DCTCP paper")
